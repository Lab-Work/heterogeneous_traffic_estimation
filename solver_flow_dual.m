function U_n = solver_flow_dual(n,U,model_param,theta)
% propagate the flow (state vector) to the next timestep given initial
% conditions and boundary conditions. "dual" version allows model
% parameters theta to change.

% theta is [vm, rm1, rm2]
% ========= modified for dual state-parameter estimation ===========
% ---1. replaced model_param. (vm1, rm1 and rm2) with theta(1,2,3)---
% ---2. add 'theta' as an argument for all the functions
% ========= modified for dual state-parameter estimation ===========
...
switch model_param.model_name
    case {'Creeping model','True model'}
        ...
        case1 = rl<=theta(3);        % left state in the domain D1
        case2 = r<=theta(3);         % middle state in the domain D1
        case3 = rr<=theta(3);        % right state in the domain D1
        ...
        for i = 1:length(idr)-1
            if idr(i)<0
                U(1,id_r(i)) = theta(3)-U(2,id_r(i));   % insert a intermediate state
            else
                Up1(1,id_r(i)) = theta(3)-Up1(2,id_r(i));   % insert a intermediate state
            end
        end
        
        U_n = U+model_param.lambda*(Num_Flux(Um1,U,model_param,theta)- ...
            Num_Flux(U,Up1,model_param,theta));
    ...
    case 'LWR'
        ...
        for i = 2:length(U1)-1
            U1_n(i-1) = U1(i) + model_param.dt/model_param.dx * (min(sending_greenshield(U1(i-1),theta(1),theta(2)),...
            receiving_greenshield(U1(i),theta(1),theta(2))) - min(sending_greenshield(U1(i),theta(1),theta(2)), ...
            receiving_greenshield(U1(i+1),theta(1),theta(2))));
            
            U2_n(i-1) = U2(i) + model_param.dt/model_param.dx * (min(sending_greenshield(U2(i-1),theta(1),theta(3)),...
            receiving_greenshield(U2(i),theta(1),theta(3))) - min(sending_greenshield(U2(i),theta(1),theta(3)), ...
            receiving_greenshield(U2(i+1),theta(1),theta(3))));
        end
        U_n = [U1_n; U2_n];
end
%============================================
...
flux_hll = HLL(Ul,Ur,model_param,num_param,theta);      % case inst2
flux_sr = Num_Flux(Ul,Ur,model_param,theta);  % sending receiving
...
function F = HLL(Ul,Ur,model_param,num_param,theta) % the HLL Riemann solver

% where v contains boundary condition of velocity
% eigenvalues:
eigns1 = lambda(Ul,model_param,num_param,theta);  % left bound
eigns2 = lambda(Ur,model_param,num_param,theta);  % right bound
...
case3 = c2<0;
% left flux
Fl = flux(Ul,model_param,theta);  % plug into left boundary velocity information
% right flux
Fr = flux(Ur,model_param,theta);
%-----------------------------------------------
% intermediate flux
...
%--------------------------------------------------------------------------
function y = lambda(U,model_param,theta)  % U = (density, w), calculate eigenvalues

r = U(1,:)+U(2,:);    % sum
u1 = vel(r,theta(1),theta(2));
u2 = max(vel(r,theta(1),theta(3)),0);
dv1 = DiffVel(r,theta(1),theta(2));
dv2 = DiffVel(r,theta(1),theta(3));
indx = find(r>theta(3));
dv2(indx) = 0;
...
%--------------------------------------------------------------
function  F = Num_Flux(Ul,Ur,model_param,theta) % numerical flux
% find sending function/first class
s_1 = sending_1(Ul,model_param,theta);
% receiving of flux/first class
r_1 = receiving_1(Ul,Ur,model_param,theta);

% find sending function
s_2 = sending_2(Ul,model_param,theta);
% receiving of flux
r_2 = receiving_2(Ul,Ur,model_param,theta);
...
function y = receiving_1(Ul,Ur,model_param,theta)

rhoc = (theta(2)-Ur(2,:))/2;  % critical density
case1 = Ur(1,:)<=rhoc;  % maximum possible
case2 = Ur(1,:)>rhoc;   % maximum
u0(1,:) = rhoc;
u0(2,:) = Ur(2,:);
q_max = traffic_flux_1(u0,model_param,theta); % maximum flow rate
%       q_max = max(traffic_flux_1(Ur,theta(1),theta(2))); % maximum flow rate
q = traffic_flux_1(Ur,model_param,theta);
y = case2.*q + case1.*q_max;
%--------------------------------------------------
% define the receiving function for 2nd vehicle class
function y = receiving_2(Ul,Ur,model_param,theta)
% when r<theta(3), non-creeping phase
r = Ur(1,:) + Ur(2,:); % total density

rhoc = max((theta(3)-Ur(1,:))/2,0);  % critical density
case1 = Ur(2,:)<=rhoc;  % maximum possible
case2 = Ur(2,:)>rhoc;   % maximum
u0(2,:) = rhoc;
u0(1,:) = Ur(1,:);
q_max = traffic_flux_2(u0,model_param,theta); % maximum flow rate
q = traffic_flux_2(Ur,model_param,theta);
y = case2.*q + case1.*q_max;
%----------------------------------------------
% define sending function for 1st vehicle class
function y = sending_1(U,model_param,theta)% sending of creeping vehicles/for two phases

rhoc = (theta(2)-U(2,:))/2;  % a vector of rhoc/3-params fd
case1 = U(1,:)<=rhoc;  % maximum possible
case2 = U(1,:)>rhoc;   % maximum
u0(1,:) = rhoc;
u0(2,:) = U(2,:);
q_max = traffic_flux_1(u0,model_param,theta); % maximum flow rate
%       q_max = max(traffic_flux_1(U,theta(1),theta(2)));
q = traffic_flux_1(U,model_param,theta);
y = case1.*q + case2.*q_max;
%------------------------------------------------
% define sending function for 2nd vehicle class
function y = sending_2(U,model_param,theta)% U = [\rho, w] two state variables
rhoc = max((theta(3)-U(1,:))/2,0);  % critical density
case1 = U(2,:)<=rhoc;    % maximum possible
case2 = U(2,:)>rhoc;     % maximum
u0(2,:) = rhoc;
u0(1,:) = U(1,:);
q_max = traffic_flux_2(u0,model_param,theta); % maximum flow rate
q = traffic_flux_2(U,model_param,theta);
y = case1.*q + case2.*q_max;
%-----------------------------------------------
function y = flux(U,model_param,theta)  % define flux function
r = U(1,:)+U(2,:);
u1 = vel(r,theta(1),theta(2));
u2 = vel(r,theta(1),theta(3));
y(1,:) = U(1,:).*u1;
y(2,:) = U(2,:).*u2;

%-------------------------------------------------------
function y = traffic_flux_1(U,model_param,theta)  % traffic flux of 1st class
rho = U(1,:)+U(2,:);
v = vel(rho,theta(1),theta(2));
y = U(1,:).*v;
%-------------------------------------------------------
function y = traffic_flux_2(U,model_param,theta) % traffic flux of 2nd class
rho = U(1,:)+U(2,:);
v = vel(rho,theta(1),theta(3));
y = U(2,:).*v;
y = max(y,0*rho);
%-------------------------------------------------------
